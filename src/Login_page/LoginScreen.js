...
const LoginScreen = () => {
  const [checkLogin, setCheckLogin] = useState("Sign Up");
  const [changefooter, setChangeFooter] = useState("Already have an account");
  const [chageState, setChangeState] = useState(true);
  function changeLogin() {
    setChangeState(!chageState);
    if (chageState) {
      setCheckLogin("Sign Up");
      setChangeFooter("Dont have an account");
    } else {
      setCheckLogin("Sign In");
      setChangeFooter("Already have an account");
    }
  }
  return (
      ...
      <div className="max-w-screen-xl m-0 sm:m-10 bg-white shadow sm:rounded-lg flex justify-center flex-1">
        <div className="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
          ...
          <div class="mt-12 flex flex-col items-center">
            <h1 class="text-2xl xl:text-3xl font-extrabold">{checkLogin}</h1>
            <div class="w-full flex-1 mt-8">
              <div class="flex flex-col items-center">
                <button class="w-full max-w-xs font-bold shadow-sm rounded-lg py-3 bg-indigo-100 text-gray-800 flex items-center justify-center transition-all duration-300 ease-in-out focus:outline-none hover:shadow focus:shadow-sm focus:shadow-outline">
                  ...
                  <span class="ml-4">{checkLogin} with Google</span>
                </button>
                ...
                <button class="w-full max-w-xs font-bold shadow-sm rounded-lg py-3 bg-indigo-100 text-gray-800 flex items-center justify-center transition-all duration-300 ease-in-out focus:outline-none hover:shadow focus:shadow-sm focus:shadow-outline mt-5">
                  ...
                  <span class="ml-4">{checkLogin} with GitHub</span>
                </button>
              </div>

              <div class="my-12 border-b text-center">
                <div class="leading-none px-2 inline-block text-sm text-gray-600 tracking-wide font-medium bg-white transform translate-y-1/2">
                  Or {checkLogin} with e-mail
                </div>
              </div>
              ...
              <div class="mx-auto max-w-xs">
                ...
                <button class="mt-5 tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-4 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
                  ...
                  <span class="ml-3">{checkLogin}</span>
                </button>
                <p class="mt-6 text-xs text-gray-600 text-center">
                  {changefooter}{" "}
                  <a
                    class="px-6 py-2 min-w-[120px] text-center text-indigo-500 border border-indigo-500 rounded hover:bg-green-600 hover:text-white active:bg-indigo-500 focus:outline-none focus:ring cursor-pointer"
                    onClick={changeLogin}
                  >
                    {checkLogin}
                  </a>
